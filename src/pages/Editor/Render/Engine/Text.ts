...
export class Text extends Node<SVGTextElement> {
  ...
  set fontSize(fontSize: number) {
    ...
    setAttribute(this._dom, "font-size", `${fontSize}`);
    this.updateTransform();
  }
  ...
  set fill(fill: string) {
    this._fill = fill;
    setAttribute(this._dom, "fill", fill);
  ...
  toJson() {
    const { x, y, rotation, type, text, fill, fontSize, } = this;
    return {
      x, y, rotation,
      type,
      text,
  ...
  fromJson(json: any) {
    const { x, y, rotation, text, fill, fontSize, } = json;
    this.x = x;
    this.y = y;
